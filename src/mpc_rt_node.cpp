...
int main(int argc, char **argv) {
  ...
  // Define Publishers
  ros::Publisher pub_inidata = rosnh.advertise<mpc_pkg::OptiData>
  ...
  Eigen::MatrixXd Traj_r_Hp_rot(Hp,3);
    ...
    tie(phi_red, dphi_red) = take_reduced_mesh(state_SOM.segment(0,3*SOMlength),
  	...
  	x_ini_COM << phi_red, dphi_red;
		...
		mpc_pkg::OptiData optiData_pub;
		optiData_pub.xinicom_rot = vector<double>(x_ini_COM_rot.data(),
												 x_ini_COM_rot.size()+x_ini_COM_rot.data());
		optiData_pub.u_rot = vector<double>(u_rot.data(), u_rot.size()+u_rot.data());
